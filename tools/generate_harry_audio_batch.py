#!/usr/bin/env python3
"""Generate many chapters in one run while keeping chapter output directories separate."""
...
def main() -> None:
    ...
    rows = sorted(unique.values(), key=lambda row: (row["sentence_id"], row["sentence_ordinal"]))
    ...
    completed = 0
    completed_by_take = {1: 0, 2: 0}

    def output_for(row: dict, take: int) -> Path:
        chapter = int(row["sentence_id"][2:5])
        output = args.stage / f"ch{chapter:02d}" / ("audio-a" if take == 1 else "audio-b")
        output.mkdir(parents=True, exist_ok=True)
        return output

    for index, row in enumerate(rows):
        for take, reference in ((1, first_refs[index]), (2, second_refs[index])):
            output = output_for(row, take)
    ...
    def status(current=None) -> None:
        ...
        atomic_json(status_path, {
            ...
            "audio_a": completed_by_take[1],
            "audio_b": completed_by_take[2],
                    ...
                    try:
                        response = session.post(
                            ...
                            data=[("prompt_texts", prompt), ("dialogue_text", f"[S1]{row['speak_text']}"),
                                  ("seed", str(args.seed + take * 100_000 + chapter * 1_000 + ordinal + retry)),
