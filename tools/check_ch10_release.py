"""Chapter 10 checks, isolated from real users' learning progress."""
import hashlib
import json
import sys
import tempfile
...
root = Path('/home/scpark/harry-concise-ch10')
payload, rows = load_validated_manifest(root / 'ch010.json')
assert len(rows) == 151
assert rows[0]['display_text'] == 'Chapter Ten. Luna Lovegood.'
assert rows[0]['speak_text'] == 'Chapter Ten. Luna Lovegood.'
assert payload['counts'] == {'sentences': 151, 'body_sentences': 150, 'takes': 302}
...
assert [r['display_text'] for r in rows[1:]] == pdf_sentences(source_pdf, 92, 99, 150)
assert [r['speak_text'] for r in rows[1:]] == tts_sentences(
    source_tts, 'Chapter Ten. Luna Lovegood.',
    "Chapter Eleven. The Sorting Hat's New Song.", 150)
proper = json.loads((root / 'ch010-proper-nouns.json').read_text())
validate_proper_nouns(proper, rows, label='Chapter 10')
# Both the title and body must retain canonical names, never TTS spelling.
assert rows[3]['display_text'] == "Mrs. Weasley cried over Kreacher's dead body."
assert rows[30]['display_text'] == 'Enemies could catch an owl and read it.'
assert rows[150]['display_text'] == 'He climbed inside beside Luna and the others.'
for ordinal, names in ((0, {'Luna', 'Lovegood'}), (58, {'Mimbulus', 'mimbletonia'}),
                       (67, {'Cho', 'Chang'}), (123, {'Grubbly-Plank'})):
    row = rows[ordinal]
...
with tempfile.TemporaryDirectory(prefix='dictai-ch10-check-') as tmp:
    server.DB = Path(tmp) / 'test.sqlite3'
    with TestClient(server.app, base_url='https://testserver') as client:
        assert client.post('/api/book', json={'chapter': 9}).status_code == 200
        assert client.post('/api/level', json={'level': 37}).status_code == 200
        assert client.post('/api/book', json={'chapter': 10}).json()['count'] == 151
        assert client.get('/api/bootstrap').json()['level'] == 1
        for ordinal in (1, 4, 15, 31, 59, 68, 89, 124, 151):
        ...
        assert client.post('/api/level', json={'level': 152}).status_code == 400
        client.post('/api/book', json={'chapter': 9})
        assert client.get('/api/bootstrap').json()['level'] == 37
        client.post('/api/book', json={'chapter': 10})
        assert client.get('/api/bootstrap').json()['level'] == 151
        server.SELECTED_BOOKS.clear()
        assert client.get('/api/bootstrap').json()['chapter'] == 10
        assert client.get('/api/bootstrap').json()['level'] == 151
