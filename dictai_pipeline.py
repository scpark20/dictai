...
def next_conversation_index(current: int, count: int) -> int:
    if count < 1:
        raise ValueError('Conversation category cannot be empty')
    choices = [number for number in range(1, count + 1) if number != current]
