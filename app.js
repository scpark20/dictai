...
const DEFAULT_MAX_LEVEL = 641;
const DEPLOYED_ORIGIN = "https://192.168.0.68:8771";
const SENTENCE_POSITION_KEY = "harry-potter-ch3-current-sentence";
...
function renderStep(level) {
  ...
  if (elements.chapterProgress) {
    ...
    elements.chapterProgress.title = `Chapter 3 progress ${percent}% · sentence ${safeLevel}/${state.maxLevel}`;
  }
...
function problemLoadingMessage(context) {
  return `Chapter 3 · ${context.level}/641 Selecting sentence`;
}
...
function clearAudioBuffer() {
  ...
  const objectUrl = state.audioObjectUrl;
...
function configureCurrentAudio(problem, version, problemLoadingContext = null, take = state.voiceTake) {
  ...
  void (async () => {
    try {
      const response = await fetch(API.audio(problem.attemptId, take), {
        credentials: "same-origin",
        cache: "no-store",
        signal: fetchController.signal,
      });
      if (!response.ok) {
        throw new ApiError("Wonder Could not load the original audio.", response.status);
      }
      const contentType = response.headers.get("content-type") || "";
      ...
      if (!isCurrent() || fetchController.signal.aborted) return;
