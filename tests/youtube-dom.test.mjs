...
let apiCalls=0,playerInstances=0,lastPlayer;
...
assert.equal($('timeline'),null);assert.equal($('showScript'),null);assert.equal($('rangeForm'),null);
...
$('forwardClip').click();assert.equal($('clipCounter').textContent,'3 / 3');
$('giveUpButton').click();assert.equal($('nextButton').disabled,true);assert.equal($('nextButton').textContent,'Video complete ✓');
const saved=JSON.parse(localStorage.getItem('dictai:youtube:v1:jNQXAC9IVRw:en:fixture1'));assert.equal(saved.index,2);assert.equal(saved.start,0);
...
$('previousClip').click();$('answerInput').value='Green';submit('answerForm');
...
assert.equal($('clipCounter').textContent,'2 / 3');
...
console.log('DOM unit checks passed: import, removed timeline, video segment bounds, matching, duplicates, reveal, Again/Next, persistence and second player mount. YouTube is mocked; not a real playback test.');
