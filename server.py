...
def youtube_page(request: Request) -> Response:
    if request.query_params.get("embed") == "1":
        ...
        source = source.replace('</head>', '<meta name="referrer" content="strict-origin-when-cross-origin"><link rel="stylesheet" href="/youtube-assets/youtube.css?v=lr-4"></head>')
        source = re.sub(r'<script src="\./app\.js[^\"]*"></script>', '<script type="module" src="/youtube-assets/youtube.js?v=lr-4"></script>', source)
