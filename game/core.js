...
export class Game {
  ...
  tick(dt){
    ...
    if(this.cursor<this.questions.length&&this.elapsed>=this.spawnAt&&this.questions.filter(q=>q.state==='falling').length<5){const q=this.questions[this.cursor++];q.state='falling';q.spawnedAt=this.elapsed;q.speed=factor*Math.min(1.35,1+Math.floor(this.elapsed/40)*.05);this.spawnAt=this.elapsed+7/factor;events.push({type:'spawn',q});}
  ...
  mark(indices,hint=false){const q=this.current;if(!q||this.status!=='running')return null;for(const i of indices){if(!q.solved.includes(i)&&i>=0&&i<q.words.length){q.solved.push(i);if(hint)q.hints.push(i);}}if(q.solved.length!==q.words.length)return null;q.state='solved';if(!q.hints.length){this.combo++;this.best=Math.max(this.best,this.combo);this.score+=100+Math.min(100,(this.combo-1)*10);}else this.combo=0;this.active=null;this.auto();this.checkEnd();return q;}
